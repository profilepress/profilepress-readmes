...
= 2.9.26 =
* Added user moderation support to tab widget
...
= 2.9.23 =
* Added missing lostpassword_post hook.
* Fixed bug where html content type for admin email was still plain text.

= 2.9.22 =
* Fix bug where password reset didn’t work in some hosting.

= 2.9.21 =
* Upgraded hybridauth
* Added extra parameter to pp_front_end_profile filter

= 2.9.20 =
* Fixed ERR_TOO_MANY_REDIRECTS error when global redirect is active.

= 2.9.19 =
* Bug fix: profile not showing up data.

= 2.9.18 =
* Fixed bug where specifying user-id to the profile shortcode didn’t work.
...
* Fixed bug where password reset with invalid reset key didn’t show an error.

= 2.9.15 =
* Fixed couple of redirection bugs.

= 2.9.14 =
* Fixed google social login asking unnecessary permissions.

= 2.9.13 =
* Fixed PHP Fatal error: Uncaught Error: Call to a member function get_page_permastruct() on null.
* Fixed google social login not working.

= 2.9.12 =
* Fixed social login with Linkedin.
* Removed duplicate filters.

= 2.9.11 =
* Fixed error triggered when username requirement is disabled.
* Fix:  PHP Deprecated:  define(): Declaration of case-insensitive constants is deprecated.
...
* Fixed bug where redirect to current page after ajax login wasn’t working.
* Fixed social login issue with pantheon hosting.
* Removed invalid Facebook login scope.
...
* Added: URL parameters or query string to populate signup field now possible.
* Improve destroying of social login session after login/logout.
* Added registration flag to social signup login redirect filter.
...
* frontend.js now uses $(document).on() for improved ajax compatibility.
* Added more param to 'pp_after_saving_avatar' filter
